...
def segmentation(Piece, d, match, scoring_fn=default_scoring_fn, start=0, dur=-1, section_prefix='', depth=0):
    ...
    with timer("Segmentation: 1/3 done"):
        score = {k: scoring_fn(k, Piece, d, match) for k in list(match.keys()) if (k[0] >= start and k[0]+k[1] <= start+dur) }
        best = sorted(list(score.keys()), key=lambda x: -score[x]) # order the keys by their score
