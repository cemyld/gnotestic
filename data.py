...
class piece(object):
    def __init__(self, filename_or_midi, filename=None, pos_offset=0):
        ...
        self.bar = fixed(self.unified_track.bar * self.unified_track.ticks)
    ...
    def compare_with(self, other):
        ...
        if True:
            ...
            ed2 = edit_distance_norm(self_tr.bar_positions, other_tr.bar_positions)
            ed3 = edit_distance_norm(self_tr.directions, other_tr.directions)
            edit_dists += [ed0, ed1, ed2, ed3]
        return edit_dists

    def __eq__(self, other):
        return isinstance(other, self.__class__) and (self.compare_with(other) == [0.0,0.0,0.0,0.0])

    def __ne__(self, other):
        return not self.__eq__(other)

    def is_similar(self, other, c, tolerance):
        temp_features = self.compare_with(other)
        return c.predict_proba(temp_features)[0][1] > (1-tolerance)

    def __hash__(self):
        return hash(self.unified_track)
    ...
    for o, a in opts:
        ...
        def compare_two_pieces():
            if len(sys.argv) >= 3:
                ...
                for k,v in list(ss.pair_dict.keys()):
