...
def identify_voices(piece, pen=None, verbose=False):
    ...
    for k in cs_keys:
        ...
        temp_best_so_far = { n:v for n, v in best_so_far.items() if (k - n.pos - n.dur) * pen.silence < pen.new_voice }
        #print temp_best_so_far
        voices = list(temp_best_so_far.keys())
        print("\nvoices", len(voices))
        for i in range(len(commons)): # add new empty voices to the mix
            voices.append(None)
...
def extract_voices(best_so_far):
    ...
    '''
    voices = []
    all_notes = list(best_so_far.keys())
    #print "all:", all_notes
    while all_notes:
